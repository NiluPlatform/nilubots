...
import static tech.nilu.bots.proxy.web3.Bip44Wallet.HDUtil.*;
...
public class Bip44Wallet {
    ...
    public Bip44Wallet(Bip39Locale lang, String pass) {
        ...
        mnemonic = MnemonicUtils.generateMnemonic(lang, initialEntropy);
        mnemonicBytes = MnemonicUtils.generateSeed(mnemonic, pass);
        seed = new HDSeed(MiscUtil.checkNotNull(mnemonicBytes)
                , System.currentTimeMillis() / 1000);
        chain = new HDKeyChain(seed);
    }
    ...
    public HDKey createKey(String path) {
        List<ChildNumber> keyPath = parsePath(path);
    ...
    static class HDUtil {
        static HDKey deriveChildKey(HDKey parent, ChildNumber childNumber) throws HDWalletException {
            ...
            return new HDKey(append(parent.getPath(), childNumber), rawKey.chainCode
    ...
    public static class HDKey extends ECKey {
        ...
        public HDKey(HDKey parent, List<ChildNumber> childNumberPath
                , int depth
        ...
        public HDKey(List<ChildNumber> childNumberPath,
                     byte[] chainCode,
                     BigInteger priv,
                     HDKey parent) {
    ...
    public static class Hierarchy {
            ...
            if (!keys.containsKey(absolutePath)) {
                if (!create)
                    throw new IllegalArgumentException(String.format(Locale.US, "No key found for %s path %s.",
                            relativePath ? "relative" : "absolute", formatPath(path)));
                ...
                putKey(deriveChildKey(parent, absolutePath.get(absolutePath.size() - 1)));
            }
        ...
        private HDKey deriveChild(HDKey parent, ChildNumber createChildNumber) {
            HDKey childKey = deriveChildKey(parent, createChildNumber);
            putKey(childKey);
            return childKey;
        }
    ...
    public static class HDKeyChain {
        ...
        public static final List<ChildNumber> EXTERNAL_PATH = concat(ACCOUNT_ZERO_PATH, EXTERNAL_SUBPATH);
        public static final List<ChildNumber> INTERNAL_PATH = concat(ACCOUNT_ZERO_PATH, INTERNAL_SUBPATH);
        // m / 44' / 0' / 0'
        ...
        public HDKeyChain(HDSeed seed) {
            ...
            rootKey = createMasterPrivateKey(MiscUtil.checkNotNull(seed.getSeed()));
